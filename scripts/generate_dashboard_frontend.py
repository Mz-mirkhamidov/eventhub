#!/usr/bin/env python3
"""Generate Eventhub dashboard frontend files."""
import os

ROOT = "/opt/eventhub/frontend/src"
...
write("components/dashboard/DashboardLayout.tsx", '''"use client";
...
export default function DashboardLayout({
  ...
  if (!auth) {
    return (
      <motion className="flex min-h-screen items-center justify-center bg-slate-50">
  ...
  return (
    <motion className="min-h-screen bg-slate-50">
      {open && (
        <button
          type="button"
      ...
      >
        <motion className="flex h-full flex-col p-4">
          ...
          <motion className="border-t border-slate-200 pt-4">
...
# Continue in next write call - file too long, split generation
